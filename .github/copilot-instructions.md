...
- **Transient/throttle errors retry with a bus-scheduled backoff — don't make them terminal.** A throttle (429), timeout or transient 5xx (`TransientErrorClassifier`) at any migrate step parks the item in the non-terminal `RetryScheduled` status ("Waiting to retry" in both UIs) and records a concrete `NextRetryAt` from the SharePoint `Retry-After` header (`ThrottleInfo`) or the exponential `ThrottleBackoff`. The retry is **scheduled directly on the Service Bus** (`IColdStorageQueuePublisher.ScheduleAsync`, `ScheduledEnqueueTime = NextRetryAt`) so it resumes even when the Function idles — it does **not** rely on the reconciler (relying on the in-process reconciler timer was the freeze bug). The `MigrationDispatchReconciler` is only a late safety net (re-drives `RetryScheduled` items well past their `NextRetryAt`). Bounded by `ColdStorageMaxProcessAttempts`, then terminal. Only *permanent* errors fail on the first hit. Don't route a transient failure straight to `CopyToColdStorageFailed`/`DeleteFailed`/`PlaceholderFailed`, and don't remove the bus-scheduling in favour of the reconciler.
- **Migrate is conflict-aware by date (`MigrateConflictResolver`).** If a file already has a cold-storage blob, compare the source's last-modified to the archive's recorded source-modified: older archive → overwrite; same version → skip the copy but still write the placeholder; newer archive → refuse (keep the source). Jobs expose an ETA (`JobEtaCalculator`: `estimatedCompletionUtc`, `throttledCount`, `nextRetryUtc`) surfaced on both UIs' progress bars.
- **Never label a source-deleted item `CopyToColdStorageFailed`; reconcile already-archived rows to completed (`ArchivedItemReconcile`).** Since the source is only deleted after a confirmed copy, an item with `SourceDeletedAt` set is past the point of no return — the give-up/stall sweeps route it to `PlaceholderFailed` (not copy-failed), and a **pass-0** in every `MigrationDispatchReconciler` pass (`JobStatusWriter.CompleteAlreadyArchivedAsync`) corrects any row that is fully archived (copied + source deleted + placeholder created) but left non-completed → `ColdStorageMigrationCompleted` (recomputing the job rollup). `RequeueAsync`/"Recover failed" short-circuits the same rows to completed (`RequeueResultResponse.Recovered`) instead of re-driving a deleted source. This fixed the multi-job-same-folder incident where hundreds of already-archived files were re-failed on every "Recover failed" because a requeue kept the original `CreatedAt` and tripped the 24h give-up. Don't reintroduce an unconditional copy-failed in those paths.
- **Restore is blob-driven (cold storage is the source of truth).** `MigrationExpander.ExpandRestoreAsync` builds the work list by enumerating the archived **blobs** under the selected folder's blob prefix (`IColdStorageBlobEnumerator`) + resolving explicit placeholders to their blob — NOT from `migration_job_items` rows (the DB is only an audit log, and can be missing/stale). Each blob self-describes its original SharePoint location via `spOriginal*` metadata (+ the blob key reverses to the path). `SharePointRestorePipeline.ProcessBlobDrivenAsync` (taken when `RestoreTarget.BlobPath` is set → `IsBlobDriven`) restores straight from the blob and removes the `.url` placeholder only **if it still exists** — so an "orphaned" archive (blob present, placeholder and/or DB row gone) is restorable. Idempotent: a destination already present at the archived size is `Skipped` (no re-copy, no conflict-fail). **Two gotchas:** enumeration MUST skip version sidecars (`VersionBlobLayout.IsVersionArtifact` — `.versions/` + `.versions.json`) or they get pushed back as junk files; and any envelope rebuilt from a DB row (`ColdStorageBusMessageFactory.BuildEnvelopeFromItem`) MUST carry `item.BlobPath`, else a reconciler re-drive of an orphan reverts to the placeholder path and terminal-fails.
- The `.url` placeholder is INI-style; build/parse it only via `Models/ColdStorage/PlaceholderFileMetadata.cs`.
- **Provider abstraction (`Migration.Engine/Providers/`, feature-flagged, off by default).** A provider-neutral rewrite of migrate + restore over `ISourceStore` (live store, e.g. SharePoint) + `IColdStore` (archive, e.g. Azure Blob), so the engine is unit-testable with in-memory adaptors and new backends can be added. Gated by the `ColdStorageUseProviderPipelines` config int (`0` = legacy inline pipelines, `>0` = neutral `MigratePipeline`/`RestorePipeline`) in `ColdStorageMessageProcessor`. The neutral pipelines keep every guard (delete-safety, verify-before-delete, idempotent retries, throttle-parking); 29 in-memory tests in `Migration.Engine.Tests/Providers/`. Merging is safe (flag off); **don't flip it in prod until the real SharePoint/Azure adaptors are integration-tested in non-prod**. Version-history + permissions restore not yet ported.
