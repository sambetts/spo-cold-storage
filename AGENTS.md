...
- **Savings live pricing calls out to the internet.** `ReportsController` (savings) optionally fetches the live Azure storage price from the **public** Azure Retail Prices API (`prices.azure.com`) via `AzureRetailPriceService` (typed `HttpClient` + `IMemoryCache`, ~10 s timeout). It's gated on `ColdStorageAzureRegion` being set (deploy defaults it to `location`), keyed by region + `ColdStorageAzureRetailSku` + `ColdStorageCurrency`, and **falls back to the configured `ColdStorageAzurePricePerGbMonth` on any failure** — never throws. SharePoint has no public regional price API, so `ColdStorageSpoPricePerGbMonth` stays a configured assumption. Pure reporting — it doesn't touch the migrate/delete pipeline. The dashboard also links each price to a source (Azure Blob pricing page / SharePoint storage-limits doc) and, when live, a `verify` link to the exact Retail Prices API query (`AzureRetailPriceService.BuildQueryUrl`).
- **Cold-storage downloads stream through the API — don't reintroduce a blob SAS redirect.** The storage account denies public network access, so the browser can't reach the blob (a user-delegation SAS redirect fails with `AuthorizationFailure`). `PlaceholdersController.DownloadAsync` (bearer + ACL) mints a short-lived, item-scoped **Data Protection** token; `ContentAsync` (`GET content/{itemId}?t=…`, `[AllowAnonymous]`) validates it and streams the blob via the Web App MSI (`OpenReadAsync`, range-enabled) over the private endpoint. Needs `AddDataProtection()` (Program.cs) and the Web App MSI's Storage Blob Data Contributor (already granted). The token-in-URL is what lets the browser's top-level navigation (no bearer) authorise.
- **Migration.Engine.Tests should be fully green** (262/0). The legacy SnapshotBuilder tests that used to fail were removed with the legacy code; a new failure is genuinely yours.
