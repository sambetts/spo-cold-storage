...
**Transient/throttle retry:** a transient error (HTTP 429 throttle, timeout, transient 5xx — classified by `TransientErrorClassifier`) at any migrate step does **not** fail the item terminally. `ColdStorageMigratorPipeline.HandleStepFailureAsync` parks it in the non-terminal **`RetryScheduled`** status (visible in the SPFx + SPA UIs as "Waiting to retry") with an **exponential backoff** (`ThrottleBackoff`, base→cap via `ColdStorageThrottleBackoff{Base,Max}Seconds`). The message is completed; the reconciler re-drives it once the backoff elapses (claim-then-publish: it flips `RetryScheduled→Queued` and saves *before* publishing, so a picked-up worker isn't clobbered). Only after `ColdStorageMaxProcessAttempts` throttled attempts does it go terminal. A *permanent* error still fails terminally on the first hit.
